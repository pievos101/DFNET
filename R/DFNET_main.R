...
#'
#' If \code{forest} is given, train it for \code{niter} iterations on
#' \code{features} and \code{target}.  Otherwise, allocate a new forest of
#' \code{ntrees} trees and train that.
#'
#' @param forest a forest as generated by previous \code{train}ings.
...
#' @param ntrees the number of trees per generation (ignored if \code{forest}
#' is given)
#' @param initial.walk_depth the initial random walk depth (ignored if \code{forest}
#' is given)
#' @param performance a function to call with a decision tree as argument to
#' estimate that tree's performance.
